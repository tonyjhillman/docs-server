...
To manage on-the-wire security settings globally, with the REST API, use the `GET`, `POST`, and `DELETE` methods with the `/settings/security` URI.
...
[#delete-per-service-security-settings-with-the-rest-api]
=== Delete Per Service Security Settings, with the REST API
