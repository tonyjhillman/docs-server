...
Note that the `check`, `count`, `dictionary`, `fields`, `mapping`, `query`, and `registry` command-values can also be used to return information on _Scorch_ index partitions.
The syntax is in each case identical to that demonstrated for _Moss_ index partitions on this page: however, note that the path to a _Scorch_ index partition must be differently specified, as described in xref:cli:cbft-bleve.adoc#specifying-index-partitions[Specifying Index Partitions].
